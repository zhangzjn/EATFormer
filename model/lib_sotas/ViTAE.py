...
import numpy as np
from model.lib_sotas.lib_vitae.vitmodules import ViTAE_ViT_basic
...
def ViTAE_basic_Small(pretrained=False, **kwargs):  # adopt performer for tokens to token
    ...
    return model
@MODEL.register_module
def ViTAE_basic_Small_384(pretrained=False, **kwargs):  # adopt performer for tokens to token
    model = ViTAE_ViT_basic(img_size=384, RC_tokens_type=['performer', 'performer', 'performer_less'], NC_tokens_type=['transformer', 'transformer', 'transformer'], stages=3, embed_dims=[64, 64, 192], token_dims=[96, 192, 384],
                        downsample_ratios=[4, 2, 2], NC_depth=[0, 0, 14], NC_heads=[1, 1, 6], RC_heads=[1, 1, 1], mlp_ratio=3., NC_group=[1, 1, 96], RC_group=[1, 1, 1], **kwargs)
    model.default_cfg = default_cfgs['ViTAE_basic_Tiny']
