...
class MixBlock(nn.Module):
	...
	def forward(self, x):
		...
		x = residual + self.drop_path(
			self.conv2(torch.sigmoid(self.sa_weight) * sa + (1 - torch.sigmoid(self.sa_weight)) * conv))
...
class VisionTransformer(nn.Module):
	...
	def __init__(self, img_size=[224, 56, 28, 14], patch_size=[4, 2, 2, 2], in_chans=3, num_classes=1000,
...
def container_v1_light(pretrained=False, **kwargs):
	model = VisionTransformer(
		...
		num_heads=16, mlp_ratio=[8, 8, 4, 4], qkv_bias=True,
