...
class DAttentionBaseline(nn.Module):
	...
	def forward(self, x):
		...
		B, C, H, W = x.size()  # torch.Size([8, 384, 14, 14])
		dtype, device = x.dtype, x.device
		
		q = self.proj_q(x)  # torch.Size([8, 384, 14, 14])
		q_off = einops.rearrange(q, 'b (g c) h w -> (b g) c h w', g=self.n_groups, c=self.n_group_channels)  # 3 * 128, torch.Size([24, 128, 14, 14])
		offset = self.conv_offset(q_off)  # B * g 2 Hg Wg  torch.Size([24, 2, 14, 14])
		Hk, Wk = offset.size(2), offset.size(3)  # 14, 14
		n_sample = Hk * Wk  # 196
		
		if self.offset_range_factor > 0:
			offset_range = torch.tensor([1.0 / Hk, 1.0 / Wk], device=device).reshape(1, 2, 1, 1)  # torch.Size([1, 2, 1, 1])
			offset = offset.tanh().mul(offset_range).mul(self.offset_range_factor)  # torch.Size([24, 2, 14, 14])
		
		offset = einops.rearrange(offset, 'b p h w -> b h w p')  # torch.Size([24, 14, 14, 2])
		reference = self._get_ref_points(Hk, Wk, B, dtype, device)  # torch.Size([24, 14, 14, 2])
		
		if self.no_off:
			offset = offset.fill(0.0)
		
		if self.offset_range_factor >= 0:
			pos = offset + reference  # torch.Size([24, 14, 14, 2])
		else:
			pos = (offset + reference).tanh()
		
		x_sampled = F.grid_sample(
			input=x.reshape(B * self.n_groups, self.n_group_channels, H, W),  # torch.Size([8*3, 128, 14, 14])
			grid=pos[..., (1, 0)],  # y, x -> x, y
			mode='bilinear', align_corners=True)  # B * g, Cg, Hg, Wg torch.Size([24, 128, 14, 14])
		
		x_sampled = x_sampled.reshape(B, C, 1, n_sample)  # torch.Size([8, 384, 1, 196])
		
		q = q.reshape(B * self.n_heads, self.n_head_channels, H * W)  # torch.Size([96, 32, 196])
		k = self.proj_k(x_sampled).reshape(B * self.n_heads, self.n_head_channels, n_sample)  # torch.Size([96, 32, 196])
		v = self.proj_v(x_sampled).reshape(B * self.n_heads, self.n_head_channels, n_sample)  # torch.Size([96, 32, 196])
		
		attn = torch.einsum('b c m, b c n -> b m n', q, k)  # B * h, HW, Ns torch.Size([96, 196, 196])
		...
		if self.use_pe:
			...
			if self.dwc_pe:
				...
				rpe_table = self.rpe_table  # torch.Size([12, 27, 27]) 14 * 2 - 1
				rpe_bias = rpe_table[None, ...].expand(B, -1, -1, -1)  # torch.Size([8, 12, 27, 27])
				
				q_grid = self._get_ref_points(H, W, B, dtype, device)  # torch.Size([24, 14, 14, 2])
				
				displacement = (
						q_grid.reshape(B * self.n_groups, H * W, 2).unsqueeze(2) - pos.reshape(B * self.n_groups,
																							   n_sample, 2).unsqueeze(
						1)).mul(0.5)  # torch.Size([24, 196, 196, 2])
				
				attn_bias = F.grid_sample(
					input=rpe_bias.reshape(B * self.n_groups, self.n_group_heads, 2 * H - 1, 2 * W - 1),
					grid=displacement[..., (1, 0)],
					mode='bilinear', align_corners=True
				)  # B * g, h_g, HW, Ns torch.Size([24, 4, 196, 196])
				
				attn_bias = attn_bias.reshape(B * self.n_heads, H * W, n_sample)  # torch.Size([96, 196, 196])
				
				attn = attn + attn_bias
		
		attn = F.softmax(attn, dim=2)  # torch.Size([96, 196, 196])
		attn = self.attn_drop(attn)
		
		out = torch.einsum('b m n, b c n -> b c m', attn, v)  # torch.Size([96, 32, 196])
		
		if self.use_pe and self.dwc_pe:
			out = out + residual_lepe
		out = out.reshape(B, C, H, W)  # torch.Size([8, 384, 14, 14])
...
def DAT_T(pretrained=False, **kwargs):
	model = DAT(img_size=224, patch_size=4, expansion=4,
				 dim_stem=96, dims=[96, 192, 384, 768], depths=[2, 2, 6, 2],
...
@MODEL.register_module
def DAT_S(pretrained=False, **kwargs):
	model = DAT(img_size=224, patch_size=4, expansion=4,
...
@MODEL.register_module
def DAT_B(pretrained=False, **kwargs):
	model = DAT(img_size=224, patch_size=4, expansion=4,
				 dim_stem=128, dims=[128, 256, 512, 1024], depths=[2, 2, 18, 2],
				 heads=[4, 8, 16, 32], window_sizes=[7, 7, 7, 7],
