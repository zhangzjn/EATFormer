...
from model.lib_sotas.lib_vitaev2.vitmodules import ViTAEv2_basic
...
def ViTAEv2_S(pretrained=False, **kwargs): # adopt performer for tokens to token
	...
	return model
@MODEL.register_module
def ViTAEv2_S_384(pretrained=False, **kwargs): # adopt performer for tokens to token
	model = ViTAEv2_basic(img_size=384, RC_tokens_type=['window', 'window', 'transformer', 'transformer'], NC_tokens_type=['window', 'window', 'transformer', 'transformer'], stages=4, embed_dims=[64, 64, 128, 256], token_dims=[64, 128, 256, 512], downsample_ratios=[4, 2, 2, 2],
							NC_depth=[2, 2, 8, 2], NC_heads=[1, 2, 4, 8], RC_heads=[1, 1, 2, 4], mlp_ratio=4., NC_group=[1, 32, 64, 128], RC_group=[1, 16, 32, 64], window_size=7, **kwargs)
	model.default_cfg = default_cfgs['ViTAE_stages3_7']
